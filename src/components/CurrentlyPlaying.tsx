  ...
  return (
    <div className="flex h-full flex-col p-4 md:p-6">
      ...
      <img
        src={coverArt}
        alt="Album Cover"
        className="w-full max-w-[520px] aspect-square object-cover rounded-xl mx-auto"
      />
      ...
      <div className="w-full max-w-[640px] mx-auto px-2 mt-4">
        ...
        <div className={`relative w-full h-1 ${hasSong ? 'bg-red-100' : 'bg-gray-400'} rounded-lg overflow-hidden`}>
          <div
            className={`absolute h-full rounded-lg transition-all ${hasSong ? getProgressColor(progress, duration) : ''}`}
            style={{ width: hasSong && duration > 0 ? `${(progress / duration) * 100}%` : '0%' }}
          />
          <input
            type="range"
            min="0"
            ...
            className={`absolute inset-0 w-full h-full opacity-0 ${hasSong ? 'cursor-pointer' : 'cursor-not-allowed'}`}
            disabled={!hasSong}
          />
        </div>
      </div>
      ...
      {/* Volume */}
      <div className="mt-4 w-full max-w-[640px] mx-auto px-2">
        <VolumeControls onVolumeChange={onVolumeChange} onLyricsToggle={onLyricsToggle} />
      </div>
    </div>
  );
